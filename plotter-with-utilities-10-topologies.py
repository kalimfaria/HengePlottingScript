...
#        print topology4_utility
     ...
     #   ax2.scatter(time, topology10_latency, edgecolors ="#ffff99", label="T10 Latency", marker="h", facecolors='none', s=40,)
        ...
        for j in range(0, len(rebalance_time)):
           # if (rebalance_time[j] >=5000 and rebalance_time[j] <= 9000):
        ...
        ax2.legend(lines + lines2, labels + labels2 , loc=9, bbox_to_anchor=(0.5, -0.1), ncol=2,prop={'size':10}) #, label=la,)#linestyles[j%4]
        ...
        #ax2.set_yscale('log')
        #plt.xlim(9000,12000)#55000)
        ax.set_ylim(0, 1.2)
        #ax2.set_ylim(0,1000)
        #ax2.set_ylim(-10000,800000)

        plt.savefig(filename+'-sample-4.png', bbox_inches='tight')
